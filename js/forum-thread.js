(() => {
  ...
  async function load(){
    ...
    $('threadTitle').textContent=t.title;$('threadMeta').textContent=`${t.category_name} · Started by ${t.author_name} · ${new Date(t.created_at).toLocaleString()}`;
  ...
  $('replyForm').addEventListener('submit',async e=>{e.preventDefault();const msg=$('replyMessage');if(!sb){msg.textContent='Connect Supabase first.';return}const {data:{user}}=await sb.auth.getUser();if(!user){location.href=`login.html?redirect=forum-thread.html?id=${encodeURIComponent(id)}`;return}msg.textContent='Posting...';const {error}=await sb.from('forum_posts').insert({thread_id:id,author_id:user.id,body:$('replyBody').value.trim()});if(error){msg.textContent=error.message;return}$('replyBody').value='';msg.textContent='Reply posted.';load()});
  load();
})();
