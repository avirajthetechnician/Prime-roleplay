(() => {
  ...
  const categories = [
    ...
    {slug:'complaints', name:'Complaints', icon:'03', description:'Submit complaints against administrators or players.', link:'complaints.html'}
  ];
  ...
  function renderCategories(){
    cats.innerHTML = categories.map(c=>`<button class="forum-category" data-category="${c.slug}"><span class="cat-icon">${c.icon}</span><h3>${c.name}</h3><p>${c.description}</p>${c.link?'<span class="type-action">Open section →</span>':''}</button>`).join('');
  ...
  async function openModal(){ if(!sb){showNotice('Supabase is not configured.',true);return;} const {data}=await sb.auth.getUser(); if(!data.user){location.href='login.html?redirect=forum.html';return;} modal.hidden=false; }
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();const msg=$('threadFormMessage');msg.textContent='Publishing...';const {data:{user}}=await sb.auth.getUser();if(!user){msg.textContent='Please log in first.';return;}const title=$('threadTitle').value.trim(),categorySlug=$('threadCategory').value,body=$('threadBody').value.trim();const category=dbCategories.find(c=>c.slug===categorySlug);if(!category){msg.textContent='Choose a valid category.';return;}const {error}=await sb.from('forum_threads').insert({title,category_id:category.id,body,author_id:user.id});if(error){msg.textContent=error.message;return;}form.reset();modal.hidden=true;await load();showNotice('Discussion published successfully.');});
  async function loadCategories(){if(!sb)return;const {data,error}=await sb.from('forum_categories').select('id,slug,name,is_locked').order('sort_order');if(!error&&data?.length){dbCategories=data;$('threadCategory').innerHTML=data.filter(c=>!c.is_locked).map(c=>`<option value="${esc(c.slug)}">${esc(c.name)}</option>`).join('');}else{$('threadCategory').innerHTML=categories.map(c=>`<option value="${c.slug}">${c.name}</option>`).join('');}}
