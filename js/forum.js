(() => {
  ...
  function timeAgo(date){ const s=Math.max(1,Math.floor((Date.now()-new Date(date))/1000)); if(s<60)return `${s}s ago`; const m=Math.floor(s/60); if(m<60)return `${m}m ago`; const h=Math.floor(m/60); if(h<24)return `${h}h ago`; const d=Math.floor(h/24); return `${d}d ago`; }
  ...
  function render(listData=threads){
    if(!listData.length){ list.innerHTML='<div class="forum-empty">No discussions found. Be the first to start one.</div>'; return; }
    list.innerHTML=listData.map(t=>`<article class="thread-row"><div class="thread-main"><a class="thread-title" href="forum-thread.html?id=${encodeURIComponent(t.id)}">${esc(t.title)}</a><div class="thread-meta"><span class="thread-category">${esc(t.category_name || t.category)}</span> Started by <strong>${esc(t.author_name || 'Prime Member')}</strong> · ${timeAgo(t.created_at)}</div></div><div class="thread-activity"><strong>${Number(t.reply_count||0)}</strong> replies</div></article>`).join('');
  }
  function filter(category){ render(threads.filter(t=>t.category===category)); status.textContent=`Showing ${category} discussions`; }
  async function load(){
    renderCategories();
    if(!sb){ status.textContent='Forum preview mode — connect Supabase to load live discussions.'; render([]); showNotice('Supabase is not configured yet. Add your Project URL and publishable key to supabase-config.js.'); return; }
    const {data,error}=await sb.from('forum_threads').select('id,title,category,category_name,author_name,reply_count,created_at').order('created_at',{ascending:false}).limit(50);
  ...
  function openModal(){ modal.hidden=false; if(!sb){ showNotice('Connect Supabase first, then sign in to create discussions.',true); return; } sb.auth.getUser().then(({data})=>{ if(!data.user){ modal.hidden=true; location.href='login.html?redirect=forum.html'; } }); }
